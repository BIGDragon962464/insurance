    ...
    <el-dialog title="角色信息" :visible.sync="dialogFormVisible" width="30%">
      <el-form label-width="80px" size="small">
        <el-form-item label="类型名称">
          <el-input v-model="form.types" auto-complete="off"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    save(){
      this.request.post("/types",this.form).then(res => {
        if (res.code === '200'){
    ...
    del(id){
      this.request.delete("/types/" + id).then(res => {
        if (res.code === '200'){
    ...
    delBatch(){
      ...
      this.request.post("/types/del/batch",ids).then(res => {
        if (res.code === '200'){
